...
class DifferentiableBandPassFilter(BaseFilter1D):
    ...
    def _init_parameters(self, init_scale):
        ...
        if init_scale == 'mel':
            ...
            pha_freqs_mel = torch.linspace(pha_low_mel, pha_high_mel, self.pha_n_bands + 1)
            pha_freqs_hz = self._mel_to_hz(pha_freqs_mel)
        else:  # linear
            pha_freqs_hz = torch.linspace(self.pha_low_hz, self.pha_high_hz, self.pha_n_bands + 1)
            
        # Initialize amplitude bands
        if init_scale == 'mel':
            amp_low_mel = self._hz_to_mel(self.amp_low_hz)
            amp_high_mel = self._hz_to_mel(self.amp_high_hz)
            amp_freqs_mel = torch.linspace(amp_low_mel, amp_high_mel, self.amp_n_bands + 1)
            amp_freqs_hz = self._mel_to_hz(amp_freqs_mel)
        else:  # linear
            amp_freqs_hz = torch.linspace(self.amp_low_hz, self.amp_high_hz, self.amp_n_bands + 1)
    ...
    def _compute_filters(self):
        ...
        for i in range(self.n_filters):
            ...
            # Method 2: Frequency-weighted normalization (compensates for frequency-dependent energy)
                ...
                freq_weight = torch.sqrt(center_freq / 100.0)  # Adjust 100.0 based on your frequency range
                band_pass = band_pass / (torch.sqrt(torch.sum(band_pass ** 2)) * freq_weight)
            ...
            # Method 3: Bandwidth-aware normalization
                ...
                band_pass = band_pass / (torch.sqrt(torch.sum(band_pass ** 2)) * torch.sqrt(bandwidth))
            
            # Additional gain correction to ensure unity passband gain
            # This is applied after the chosen normalization method
            import numpy as np
            from scipy import signal as scipy_signal
            w, h = scipy_signal.freqz(band_pass.detach().cpu().numpy(), worN=8192, fs=self.fs)
