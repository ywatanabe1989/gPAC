...
class SyntheticDataGenerator:
    ...
    def generate_no_pac_signal(
        self, freq1: float = 10.0, freq2: float = 80.0, noise_level: float = 0.1
    ) -> np.ndarray:
        """
    ...
    def generate_multi_pac_signal(
        self, pac_pairs: List[Tuple[float, float, float]], noise_level: float = 0.1
    ) -> np.ndarray:
        """
        ...
        if include_no_pac:
            for _ in range(n_no_pac):
                ...
                signal = self.generate_no_pac_signal(
                    freq1=freq1, freq2=freq2, noise_level=noise
                )
    ...
    def create_torch_dataset(
        self, signals: np.ndarray, labels: np.ndarray, metadata: Optional[Dict] = None
    ) -> SyntheticPACDataset:
        """
        ...
        return SyntheticPACDataset(
            signals=signals_tensor, labels=labels_tensor, metadata=metadata_tensors
        )
...
def generate_pac_signal(
    ...
    generator = SyntheticDataGenerator(
        fs=fs, duration_sec=duration, random_seed=random_seed
    )
