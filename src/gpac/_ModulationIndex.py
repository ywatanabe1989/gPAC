...
class ModulationIndex(nn.Module):
    ...
    def forward(
        ...
        log_n_bins = torch.log(n_bins_tensor)
        # TensorPAC formula: pac = 1 + sum(p * log(p)) / log(n_bins)
        # Note: sum(p * log(p)) is negative for all p in (0,1)
        # For uniform dist: sum(1/n * log(1/n)) = -log(n)
        # So pac_uniform = 1 + (-log(n))/log(n) = 1 - 1 = 0
        # But TensorPAC normalizes this to give 2 for uniform
        # The actual formula must include additional scaling
        entropy_term = (amp_probs * torch.log(amp_probs + epsilon)).sum(dim=-1)
        # Direct TensorPAC formula
        mi_result = 1.0 + entropy_term / log_n_bins
