...
class ModulationIndex(nn.Module):
    ...
    def __init__(self, n_bins: int = 18, temperature: float = 0.1) -> None:
        """
        ...
        self.register_buffer("phase_bins", torch.linspace(-np.pi, np.pi, n_bins + 1))
        self.register_buffer(
            "phase_bin_centers", (self.phase_bins[1:] + self.phase_bins[:-1]) / 2
        )
        self.register_buffer("uniform_entropy", torch.tensor(np.log(n_bins)))
    ...
    def forward(
        ...
        if phase.shape[3] != amplitude.shape[3]:
            raise ValueError(
                f"Segment dimension mismatch: phase={phase.shape[3]}, amplitude={amplitude.shape[3]}"
            )
        ...
        for b in range(batch):
            for c in range(channels):
                ...
                weights_bc = weights_shaped[
                    b, c
                ]  # (freqs_phase, segments, time, n_bins)
        ...
        # Stack results
        mi_tensor = torch.stack(mi_list).reshape(
            batch, channels, freqs_phase, freqs_amplitude
        )
    ...
    def _compute_mi_broadcast(self, weights, amplitude, compute_distributions):
        ...
        for seg in range(segments):
            ...
            # Weighted amplitude
            weighted_amp = (
                seg_weights_t * seg_amp_exp
            )  # (freqs_p, freqs_a, n_bins, time)
            ...
            # Compute entropy
            entropy = -torch.sum(
                amp_dist * torch.log(amp_dist), dim=-1
            )  # (freqs_p, freqs_a)
        ...
        # Stack and average
        mi_per_segment_stack = torch.stack(
            mi_per_segment, dim=-1
        )  # (freqs_p, freqs_a, segments)
        mi = mi_per_segment_stack.mean(dim=-1)  # (freqs_p, freqs_a)

        if compute_distributions:
            # Compute overall distributions
            weights_flat = weights.reshape(
                freqs_phase, -1, n_bins
            )  # (freqs_p, S*T, n_bins)
            weights_flat_t = weights_flat.transpose(1, 2).unsqueeze(
                1
            )  # (freqs_p, 1, n_bins, S*T)
            ...
            weighted_amp_all = (
                weights_flat_t * amp_flat_exp
            )  # (freqs_p, freqs_a, n_bins, S*T)
            amp_sum_all = weighted_amp_all.sum(dim=-1)  # (freqs_p, freqs_a, n_bins)

            weight_sum_all = (
                weights_flat.sum(dim=1, keepdim=True) + self.epsilon
            )  # (freqs_p, 1, n_bins)
            amp_dist_all = amp_sum_all / weight_sum_all
            amp_dist_all = amp_dist_all / (
                amp_dist_all.sum(dim=-1, keepdim=True) + self.epsilon
            )
