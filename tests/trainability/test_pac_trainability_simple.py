...
def test_sincnet_filter_trainability():
    ...
    print(
        f"Target frequency {target_freq} Hz is {'inside' if final_band[0] <= target_freq <= final_band[1] else 'outside'} the learned band"
    )
    ...
    print(
        f"Distance to target reduced from {abs(initial_center - target_freq):.1f} Hz to {abs(final_center - target_freq):.1f} Hz"
    )
...
def test_pac_with_known_coupling():
    ...
    with torch.no_grad():
        ...
        print(f"\nHighest PAC at:")
        print(
            f"  Phase band {max_pha_idx}: {bands['pha_bands'][max_pha_idx].numpy()} Hz"
        )
