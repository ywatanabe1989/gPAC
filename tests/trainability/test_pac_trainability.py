...
class TestPACTrainability:
    ...
    def test_band_optimization(self):
        ...
        for epoch in tqdm(range(n_epochs)):
            ...
            x = (
                torch.tensor(np.array(batch_signals), dtype=torch.float32)
                .unsqueeze(1)
                .to(device)
            )
            ...
            if hasattr(pac_model.bandpass, "get_regularization_loss"):
                reg_losses = pac_model.bandpass.get_regularization_loss(
                    lambda_overlap=0.001, lambda_bandwidth=0.001
                )
            ...
            if (epoch + 1) % 20 == 0:
                ...
                if hasattr(pac_model.bandpass, "get_filter_banks"):
                    ...
                    print(
                        f"Current phase bands: {current_bands['pha_bands'][:2].cpu().numpy()}"
                    )
                    print(
                        f"Current amp bands: {current_bands['amp_bands'][:2].cpu().numpy()}"
                    )
        ...
        if hasattr(pac_model.bandpass, "get_filter_banks"):
            ...
            print(
                f"  Phase: {final_bands['pha_bands'][best_pha_idx].cpu().numpy()} Hz (true: {true_phase_band})"
            )
            print(
                f"  Amplitude: {final_bands['amp_bands'][best_amp_idx].cpu().numpy()} Hz (true: {true_amp_band})"
            )
        ...
        with torch.no_grad():
            pac_signal_result = pac_model(
                torch.tensor(test_pac_signal, dtype=torch.float32)
                .unsqueeze(0)
                .unsqueeze(0)
                .to(device)
            )
            noise_result = pac_model(
                torch.tensor(test_noise, dtype=torch.float32)
                .unsqueeze(0)
                .unsqueeze(0)
                .to(device)
            )
        ...
        assert (
            max_pac_signal > max_pac_noise * 2
        ), "Trained model doesn't distinguish PAC from noise"
