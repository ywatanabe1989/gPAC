...
class TestBandPassFilter:
    ...
    def test_forward_pass_3d_input(self):
        ...
        # Assert
        expected_shape = (batch_size, n_segments, self.pha_n_bands + self.amp_n_bands, self.seq_len)
    ...
    def test_input_shape_validation(self):
        ...
        for shape in valid_shapes:
            x = torch.randn(*shape)
            output = filter_obj(x)
            assert output.shape[:-1] == shape[:-1] + (self.pha_n_bands + self.amp_n_bands,)
    ...
    def test_invalid_input_shapes(self):
        ...
        assert output_1d.shape == (1, 1, self.pha_n_bands + self.amp_n_bands, self.seq_len)
        
        # 2D input (batch, seq_len) -> (batch, 1, seq_len) 
        x_2d = torch.randn(2, self.seq_len)
        output_2d = filter_obj(x_2d)
        assert output_2d.shape == (2, 1, self.pha_n_bands + self.amp_n_bands, self.seq_len)
