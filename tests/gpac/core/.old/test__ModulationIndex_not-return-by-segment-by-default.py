#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-06-08 00:01:06 (ywatanabe)"
...
def test_modulation_index_per_segment():
    """Test MI computation with per-segment results."""
    mi_calc = ModulationIndex(n_bins=18)
    ...
    result = mi_calc(phase, amplitude, return_per_segment=True)

    assert "mi_per_segment" in result
    assert result["mi_per_segment"] is not None
    assert result["mi_per_segment"].shape == (
        batch_size,
        channels,
        freqs_phase,
        freqs_amplitude,
...
def test_surrogate_computation():
    ...
    surrogate_result = mi_calc.compute_surrogates(
        phase, amplitude, n_perm=n_perm, pac_values=pac_values
    )
