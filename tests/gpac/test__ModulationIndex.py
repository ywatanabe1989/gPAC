...
class TestModulationIndex:
    ...
    def test_forward_pass_basic(self):
        ...
        phase = torch.randn(self.batch_size, self.n_channels, self.n_freqs_phase, 
                           self.n_segments, self.seq_len)
        # Wrap phase to [-pi, pi]
        phase = torch.atan2(torch.sin(phase), torch.cos(phase))
        amplitude = torch.abs(torch.randn(self.batch_size, self.n_channels, 
        ...
        # Check shapes
        assert output['mi'].shape == (self.batch_size, self.n_channels, 
                                     self.n_freqs_phase, self.n_freqs_amplitude)
        assert output['mi_per_segment'].shape == (self.batch_size, self.n_channels, 
                                                  self.n_freqs_phase, self.n_freqs_amplitude, 
                                                  self.n_segments)
        assert output['amplitude_distributions'].shape == (self.batch_size, self.n_channels, 
                                                          self.n_freqs_phase, self.n_freqs_amplitude, 
                                                          self.n_bins)
    ...
    def test_output_shapes_consistency(self):
        ...
        for batch, ch, f_pha, f_amp, seg, time in test_cases:
            ...
            assert output['amplitude_distributions'].shape == (batch, ch, f_pha, f_amp, self.n_bins)
