...
class TestTensorPACCompatibility:
    ...
    def test_explicit_frequency_bands_match(self):
        ...
        for i in range(len(f_pha_bands) - 1):
            assert f_pha_bands[i, 1] == f_pha_bands[i+1, 0], "Phase bands should be sequential"
            
        for i in range(len(f_amp_bands) - 1):
            assert f_amp_bands[i, 1] == f_amp_bands[i+1, 0], "Amplitude bands should be sequential"
    ...
    def test_string_configuration_differences(self):
        ...
        assert pac_values_string.shape == (30, 30), "String 'mres' should produce 30x30 matrix"
        ...
        assert pac_values_explicit.shape == (10, 10), "Explicit bands should produce 10x10 matrix"
    ...
    def test_pac_values_correlation(self):
        ...
        # Check shapes match
        assert pac_values_gpac.shape == pac_values_tp.shape, "PAC matrices should have same shape"
        ...
        print(f"TensorPAC range: [{pac_values_tp.min():.3f}, {pac_values_tp.max():.3f}]")
    ...
    def test_peak_detection_consistency(self):
        ...
        signal, _ = self.generate_pac_signal(phase_freq=10, amp_freq=80, coupling_strength=0.8)
        ...
        print(f"\nPeak detection:")
        print(f"gPAC peak at: {peak_pha_gpac:.1f}Hz (phase), {peak_amp_gpac:.1f}Hz (amp)")
        print(f"TensorPAC peak at: {peak_pha_tp:.1f}Hz (phase), {peak_amp_tp:.1f}Hz (amp)")
        ...
        assert abs(peak_pha_gpac - 10) < 2 * pha_band_width, "gPAC should detect phase near 10Hz"
        assert abs(peak_amp_gpac - 80) < 2 * amp_band_width, "gPAC should detect amplitude near 80Hz"
    ...
    def test_multichannel_consistency(self):
        ...
        # Add batch dimension
        signals_torch = torch.tensor(signals, dtype=torch.float32).unsqueeze(0)  # (1, n_channels, n_samples)
        ...
        assert max_pac_gpac[0] < max_pac_gpac[1] < max_pac_gpac[2], "gPAC should show increasing PAC"
        assert max_pac_tp[0] < max_pac_tp[1] < max_pac_tp[2], "TensorPAC should show increasing PAC"
