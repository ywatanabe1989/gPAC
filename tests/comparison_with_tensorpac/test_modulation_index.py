...
class TestModuleLevelComparison:
    ...
    def test_hilbert_transform_comparison(self):
        ...
        tp_pha_complex, _ = pac_signals_wavelet(
            self.signal, self.fs, 8, 12
        )
        tp_phase = np.angle(tp_pha_complex)
        
        # Amplitude from amplitude-filtered signal
        tp_amp_complex, _ = pac_signals_wavelet(
            self.signal, self.fs, 75, 85
        )
        ...
        phase_diff_gpac_tp = np.mean(np.abs(np.angle(np.exp(1j*(gpac_phase - tp_phase)))))
        ...
        # Scale comparison
        print(f"  Amplitude scale ratio TP/gPAC: {tp_amplitude.mean()/gpac_amplitude.mean():.2f}")
    ...
    def test_modulation_index_comparison(self):
        ...
        manual_mi = np.sum(amp_dist_manual * np.log(amp_dist_manual / uniform_dist + 1e-10))
