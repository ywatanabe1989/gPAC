...
class TestBandpassFilterComparison:
    ...
    def test_single_band_comparison(self):
        ...
        morlet_result = self.morlet_filter(
            self.signal, freq_center, self.fs, width=7
        )
    ...
    def test_multiple_bands(self):
        ...
        for i, (low, high) in enumerate(pha_bands):
            ...
            print(f"    Band {low}-{high} Hz: corr={corr:.3f}, power_ratio={power_ratio:.2f}")
    ...
    def test_filter_response(self):
        ...
        impulse_torch = torch.tensor(impulse, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        gpac_response = gpac_filter(impulse_torch)[0, 0, 0].numpy()
        
        # 2. Butterworth filter response
        butter_response = self.butter_bandpass_filter(impulse, freq_low, freq_high, self.fs)
        ...
        if len(gpac_3db) > 0:
            print(f"    gPAC -3dB band: {freqs[gpac_3db[0]]:.1f}-{freqs[gpac_3db[-1]]:.1f} Hz")
        
        if len(butter_3db) > 0:
            print(f"    Butterworth -3dB band: {freqs[butter_3db[0]]:.1f}-{freqs[butter_3db[-1]]:.1f} Hz")
    ...
    def test_edge_artifacts(self):
        ...
        test_signal[burst_start:burst_end] = np.sin(2 * np.pi * 10 * 
                                                    np.arange(burst_end - burst_start) / self.fs)
        ...
        signal_torch = torch.tensor(test_signal, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        ...
        print(f"      gPAC RMS: {np.sqrt(np.mean(gpac_filtered[:edge_samples]**2)):.4f}")
        print(f"      Butterworth RMS: {np.sqrt(np.mean(butter_filtered[:edge_samples]**2)):.4f}")
        ...
        trans_region = 20
        gpac_trans_start = np.max(np.abs(gpac_filtered[burst_start-trans_region:burst_start+trans_region]))
        butter_trans_start = np.max(np.abs(butter_filtered[burst_start-trans_region:burst_start+trans_region]))
