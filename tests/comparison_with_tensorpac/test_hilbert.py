...
class TestModuleLevelComparisonHilbert:
    ...
    def test_bandpass_filter_comparison(self):
        ...
        print(
            f"  Phase power ratio (Butter/gPAC): {butter_pha_power/gpac_pha_power:.2f}"
        )
        print(
            f"  Amplitude power ratio (Butter/gPAC): {butter_amp_power/gpac_amp_power:.2f}"
        )
    ...
    def test_hilbert_transform_comparison(self):
        ...
        print(
            f"  Phase extraction: min={gpac_phase.min():.3f}, max={gpac_phase.max():.3f}"
        )
        print(
            f"  Amplitude extraction: mean={gpac_amplitude.mean():.3f}, std={gpac_amplitude.std():.3f}"
        )
        ...
        print(
            f"  Phase extraction: min={scipy_phase.min():.3f}, max={scipy_phase.max():.3f}"
        )
        print(
            f"  Amplitude extraction: mean={scipy_amplitude.mean():.3f}, std={scipy_amplitude.std():.3f}"
        )
        ...
        phase_diff_gpac_butter = np.mean(
            np.abs(np.angle(np.exp(1j * (gpac_phase - butter_phase))))
        )

        print(f"  Mean phase difference gPAC-SciPy: {phase_diff_gpac_scipy:.3f} rad")
        print(
            f"  Mean phase difference gPAC-Butter+SciPy: {phase_diff_gpac_butter:.3f} rad"
        )
        ...
        print(
            f"  Amplitude scale ratio Butter+SciPy/gPAC: {butter_amplitude.mean()/gpac_amplitude.mean():.2f}"
        )
    ...
    def test_modulation_index_comparison(self):
        ...
        test_cases = [
            (
                "gPAC-gPAC",
                hilbert_results["gpac_phase"],
                hilbert_results["gpac_amplitude"],
            ),
            (
                "SciPy-SciPy",
                hilbert_results["scipy_phase"],
                hilbert_results["scipy_amplitude"],
            ),
            (
                "Butter-Butter",
                hilbert_results["butter_phase"],
                hilbert_results["butter_amplitude"],
            ),
        ]
        ...
        for name, phase, amplitude in test_cases:
            ...
            # Prepare tensors
            phase_tensor = torch.tensor(phase, dtype=torch.float32).reshape(
                1, 1, 1, 1, -1
            )
        ...
        for name, (gpac_mi, manual_mi) in mi_values.items():
            print(
                f"  {name}: gPAC={gpac_mi:.4f}, Manual={manual_mi:.4f}, ratio={manual_mi/gpac_mi:.2f}"
            )
    ...
    def test_full_pac_comparison(self):
        ...
        print(
            "  3. MI calculation: Similar algorithm, possible normalization differences"
        )
