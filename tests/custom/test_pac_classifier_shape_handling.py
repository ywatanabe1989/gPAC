...
class TestPACClassifierShapeHandling:
    ...
    @pytest.fixture
    def simplified_pac_classifier(self):
        """Shared fixture for SimplifiedPACClassifier."""

        class SimplifiedPACClassifier(nn.Module):
            def __init__(
                self, pac_module, n_pha_classes, n_amp_classes, actual_feature_dim
            ):
                super().__init__()
        ...
        return SimplifiedPACClassifier
    ...
    def test_pac_module_output_shape(self, setup_parameters):
        ...
        assert (
            output.shape[0] == params["batch_size"]
        ), f"Batch dimension mismatch: {output.shape[0]} != {params['batch_size']}"
        assert (
            output.shape[1] == params["n_channels"]
        ), f"Channel dimension mismatch: {output.shape[1]} != {params['n_channels']}"

        # Output shape is documented in test assertion messages
    ...
    def test_pac_classifier_with_multiple_channels(
        self, setup_parameters, simplified_pac_classifier
    ):
        ...
        classifier = simplified_pac_classifier(
    ...
    def test_pac_classifier_with_single_channel(
        self, setup_parameters, simplified_pac_classifier
    ):
        ...
        with pytest.raises(ValueError, match="Flattened PAC values shape mismatch"):
            ...
            class MismatchedPACClassifier(nn.Module):
                def __init__(self, pac_module):
                    ...
                    self.amp_n_bands = (
                        1  # Amplitude frequency dimension is 1 in actual output
                    )

                    n_features = (
                        self.pha_n_bands
                    )  # Only pha bands matter in actual output
                    self.classifier = nn.Linear(n_features, 5)

                def forward(self, x):
                    pac_values = self.pac_module(x)

                    # Actual shape is used for validation below
                    ...
                    if len(pac_values.shape) == 3:  # (B, C, F_pha)
                        ...
                        if pac_values.shape[1] > 1:
                            ...
                            pac_values = pac_values.squeeze(
                                1
                            )  # Remove channel dim if only one
