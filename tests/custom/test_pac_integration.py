...
class TestPACIntegration:
    ...
    def test_filtfilt_mode(self):
        ...
        fs = 512  # Higher sampling rate for better frequency resolution
        seq_len = 2048  # Longer signal for better frequency resolution
        ...
        result_standard = pac_standard(signal)
        result_filtfilt = pac_filtfilt(signal)
        pac_values_standard = result_standard['mi']
        pac_values_filtfilt = result_filtfilt['mi']
    ...
    def test_edge_mode(self):
        ...
        # Both should work
        result_no_edge = pac_no_edge(signal)
        result_reflect = pac_reflect(signal)
        pac_values_no_edge = result_no_edge['mi']
        pac_values_reflect = result_reflect['mi']
    ...
    def test_permutation_testing(self):
        ...
        fs = 512  # Higher sampling rate for better frequency resolution
        seq_len = 1024  # Adequate signal length
        ...
        # Check shapes
        assert result['mi'].shape == (1, 1, 5, 5)
        assert result['mi_z'].shape == (1, 1, 5, 5)
        assert result['surrogate_mis'].shape == (20, 1, 1, 5, 5)
        
    def test_calculate_pac_function(self):
        """Test basic PAC calculation without high-level function."""
        fs = 512  # Higher sampling rate for better frequency resolution
        seq_len = 2048  # Longer signal for better frequency resolution
        
        signal = torch.randn(2, 2, 1, seq_len)  # (batch, channel, segments, time)
        
        # Use PAC class directly
        pac_model = gpac.PAC(
            seq_len=seq_len,
            fs=fs,
            pha_start_hz=2.0,
        ...
        result = pac_model(signal)
        pac_values = result['mi']
        pha_freqs = result['pha_freqs_hz']
        amp_freqs = result['amp_freqs_hz']
    ...
    def test_device_compatibility(self, setup_pac):
        ...
        # CPU computation
        result_cpu = pac_model(signal)
        pac_cpu = result_cpu['mi']
        
        # GPU computation if available
        if torch.cuda.is_available():
            pac_model_gpu = pac_model.cuda()
            signal_gpu = signal.cuda()
            result_gpu = pac_model_gpu(signal_gpu)
            pac_gpu = result_gpu['mi']
