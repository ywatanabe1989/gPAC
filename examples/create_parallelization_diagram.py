...
def create_parallelization_diagram():
    ...
    for i, (color, name) in enumerate(zip(colors, band_names)):
        ...
        rect = patches.Rectangle((x, y_pos-0.4), band_width-0.1, 0.8, 
                               facecolor=color, edgecolor='black', linewidth=1)
        ax1.add_patch(rect)
        ax1.text(x + (band_width-0.1)/2, y_pos, name, ha='center', va='center', 
                fontsize=10, fontweight='bold')
    
    # Total time arrow
    ax1.annotate('', xy=(x_start + len(colors) * band_width - 0.1, y_pos - 1.5), 
                xytext=(x_start, y_pos - 1.5),
                arrowprops=dict(arrowstyle='<->', color='red', lw=2))
    ax1.text(5, y_pos - 2, 'Total Time = Sum of all bands', ha='center', 
            fontsize=12, color='red', fontweight='bold')
    
    # Bottleneck explanation
    ax1.text(5, 6, 'Each band waits for\nprevious band to complete', 
            ha='center', va='center', fontsize=11, 
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightcoral', alpha=0.7))
    
    ax1.text(5, 3.5, '1 CPU Core\n↓\nSequential Processing\n↓\nSlow', 
    ...
    for i, (color, name, y) in enumerate(zip(colors, band_names, y_positions)):
        rect = patches.Rectangle((x_pos, y-0.3), band_width, 0.6, 
                               facecolor=color, edgecolor='black', linewidth=1)
        ax2.add_patch(rect)
        ax2.text(x_pos + band_width/2, y, name, ha='center', va='center', 
                fontsize=10, fontweight='bold')
    
    # Short time arrow
    ax2.annotate('', xy=(x_pos + band_width, 3.5), xytext=(x_pos, 3.5),
                arrowprops=dict(arrowstyle='<->', color='green', lw=2))
    ax2.text(5, 3, 'Total Time = Single band time', ha='center', 
            fontsize=12, color='green', fontweight='bold')
    
    # Parallel explanation
    ax2.text(8.5, 7, 'All bands\ncompute\nsimultaneously', 
            ha='center', va='center', fontsize=11,
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightgreen', alpha=0.7))
    
    ax2.text(1.5, 1.5, 'Thousands of GPU Cores\n↓\nParallel Processing\n↓\n5-100x Faster', 
            ha='center', va='center', fontsize=12, fontweight='bold',
    ...
    plt.tight_layout()
    plt.savefig('/home/ywatanabe/proj/gPAC/docs/parallelization_diagram.png', 
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    
    print("Parallelization diagram saved to: /home/ywatanabe/proj/gPAC/docs/parallelization_diagram.png")
...
def create_pac_computation_flow():
    ...
    ax.set_title('PAC (Modulation Index) Computation: Independent Frequency Bands', 
                fontsize=16, fontweight='bold')
    ax.set_xlim(0, 12)
    ax.set_ylim(0, 14)
    
    # Input signal
    ax.text(6, 13, 'Input EEG Signal', ha='center', fontsize=14, fontweight='bold',
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightblue'))
    
    # Phase and amplitude extraction
    ax.text(3, 11.5, 'Phase Extraction\n(Low Frequencies)', ha='center', fontsize=12,
            bbox=dict(boxstyle="round,pad=0.3", facecolor='#FFE5B4'))
    
    ax.text(9, 11.5, 'Amplitude Extraction\n(High Frequencies)', ha='center', fontsize=12,
            bbox=dict(boxstyle="round,pad=0.3", facecolor='#E5FFE5'))
    
    # Arrows
    ax.annotate('', xy=(3, 11), xytext=(5, 12.5),
                arrowprops=dict(arrowstyle='->', lw=2, color='orange'))
    ax.annotate('', xy=(9, 11), xytext=(7, 12.5),
    ...
    for i, p_band in enumerate(phase_bands):
        for j, a_band in enumerate(amp_bands):
            ...
            ax.add_patch(rect)
            ax.text(x, y, f'MI\n{p_band}\n{a_band}', ha='center', va='center', 
                   fontsize=8, fontweight='bold')
    
    # Independence note
    ax.text(6, 0.5, 'Key Insight: Each MI calculation is INDEPENDENT\n' +
    ...
    plt.tight_layout()
    plt.savefig('/home/ywatanabe/proj/gPAC/docs/pac_computation_flow.png', 
                dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    
    print("PAC computation flow diagram saved to: /home/ywatanabe/proj/gPAC/docs/pac_computation_flow.png")
