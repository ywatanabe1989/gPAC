#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Timestamp: "2025-06-08 11:49:38 (ywatanabe)"
...
def demo_static_pac(args):
    ...
    if (
        ...
        gt_info = metadata["pac_components"][sample_idx]
...
def demo_trainable_pac(args):
    ...
    pac_analyzer.eval()
    with torch.no_grad():
        initial_pac_result = pac_analyzer(signal)
        initial_loss = -initial_pac_result["pac"].mean()
        initial_reg_loss = pac_analyzer.get_filter_regularization_loss()
        initial_total_loss = initial_loss + 0.1 * initial_reg_loss

    # Optimization loop
    losses = [initial_total_loss.item()]
    pac_analyzer.train()
    ...
    ax_pac.set_xyt(
        "Amplitude Frequency [Hz]", "Phase Frequency [Hz]", "Trainable PAC"
    )
    ax_pac.set_xticks(range(0, len(pac_analyzer.amp_mids), 20))
    ax_pac.set_xticklabels([f"{f:.0f}" for f in pac_analyzer.amp_mids[::20]])
    ax_pac.set_yticks(range(0, len(pac_analyzer.pha_mids), 20))
    ...
    # Training loss
    ax_loss = axes[0, 2]
    ax_loss.plot(range(-1, len(losses) - 1), losses, "o-")
    ax_loss.set_xyt("Epoch", "Loss", "Training Loss")
    ax_loss.set_xticks(range(-1, len(losses) - 1, 2))
